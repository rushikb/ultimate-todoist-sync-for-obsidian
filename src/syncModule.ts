...
export class TodoistSync  {
    ...
    async deletedTaskCheck(file_path:string): Promise<void> {
        ...
        if(file_path){
            ...
            filepath = file_path
            currentFileValue = await this.app.vault.read(file as TFile)
        }
        else{
    ...
    async  syncCompletedTaskStatusToObsidian(unSynchronizedEvents: any) {
        ...
        for (const e of unSynchronizedEvents) {   //如果要修改代码，让completeTaskInTheFile(e.objectId)按照顺序依次执行，可以将Promise.allSettled()方法改为使用for...of循环来处理未同步的事件。具体步骤如下：
            //console.log(`正在 complete ${e.objectId}`)
    ...
    async  syncUncompletedTaskStatusToObsidian(unSynchronizedEvents: any) {
        ...
        // 处理未同步的事件并等待所有处理完成
        const processedEvents = []
        for (const e of unSynchronizedEvents) {   //如果要修改代码，让uncompleteTaskInTheFile(e.objectId)按照顺序依次执行，可以将Promise.allSettled()方法改为使用for...of循环来处理未同步的事件。具体步骤如下：
            //console.log(`正在 uncheck task: ${e.objectId}`)
